...
- **dbt MetricFlow / AtScale** provide deterministic metric computation but only within the BI/analytics layer -- not across the full agent interaction surface.
...
> Enable a scalable (10 to 100 to 1000 datasets) agentic data access layer that is safe, accurate, and trustable -- on the existing legacy data estate -- in minimum possible time. The solution must sit between agent platforms (Frontier, Cowork, custom) and data platform semantic layers (Snowflake SVA, dbt MetricFlow), providing the deep cross-estate mediation that neither can deliver alone.
...
| Protocol-Native | Exposes context via MCP, OSI, REST/gRPC for any agent framework | Works with Frontier, Cowork, Claude Code, custom agents |
...
- Agents must respect existing access control — they cannot see data the requesting user wouldn't be authorized to access directly.
- Identity must pass through from the agent platform (JWT/OAuth) to the semantic layer (row-level security).
...
5. **Separation of Concerns:** The Context Registry (knowledge about data) is separate from the Semantic Layer (computation on data) is separate from the Decision Trace Graph (history of resolutions). Agents reason using the first, act through the second, and learn from the third.
...
7. **Protocol-Native Distribution:** Context is exposed via MCP, OSI, and REST/gRPC so any agent framework can consume it. ECP is not an agent platform; it is the intelligence layer that agent platforms need.

8. **Migration-Resilient Contracts:** Semantic Contracts are the stable interface while physical source references version underneath. Agents never break when the estate changes.

---

## Knowledge Asset Taxonomy

Agents need access to multiple types of knowledge assets:
...
|     (MCP Server / REST API / gRPC -- any agent framework connects)     |
+------------------------------------------------------------------------+
                                 |
                                 v
+========================================================================+
...
- **Semantic Layer:** Holds executable logic. What actually computes answers. Agent calls it; never bypasses it.
...
- Support industry-standard protocols (MCP, OSI, REST) for agent integration
...
- enterprise-context-platform-spec-v3.md -- Complete technical specification
