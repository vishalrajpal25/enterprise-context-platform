# Enterprise Context Platform: Technical Architecture v3

**Version: 3.0 | March 2026 | Audience: Engineering & Architecture Teams | Status: Implementation-Ready (Phases 1-3), Research Preview (Phases 4-5)**
...
| Orchestration | MCP Server | Node.js + MCP SDK | 3000/SSE | Agent integration (Frontier, Cowork, custom) |
...
### 1.3 Protocol Integration Layer (NEW)
...
|  agent (Claude,          tool (dbt, Looker,      (custom agents,       |
|  GPT, custom)            ThoughtSpot, Sigma)     dashboards)           |
+------------------------------------------------------------------------+
```
...
|   - MCP Server (available to agents)                        |
...
|                                                              |
| Output: Live, queryable, agent-consumable context            |
+-------------------------------------------------------------+
```
...
| **Decision Trace Learning** | Feedback loops | Continuous online learning from agent execution traces | HIGH: Context layer that writes its own semantic contracts | 12-18 months |
...
| Agent frameworks supported via MCP | 1 | 2 | 3+ | 5+ |
| Resolution intelligence improvement (accuracy delta from traces) | - | - | +2% | +5%+ |

---

## 8. WHAT MAKES ECP v3 UNIQUE
...
3. **Semantic Firewall** -- agents never touch raw data
4. **Agent Never Does Math** -- deterministic computation only
...
10. **Protocol-Native** -- MCP, OSI, REST for any agent framework
11. **Semantic Contracts** as the unit of work
12. **Research-ready architecture** -- designed for progressive intelligence
13. **Connector Framework** -- tiered ingestion from heterogeneous estates
14. **Entitlements Architecture** -- inherited access control with OPA + audit
15. **Observability & Evals** -- tracing, metrics, golden query suites, hallucination circuit breakers
...
```
Agent Platform (Frontier/Cowork/Custom)
...
1. **Confidence threshold (< 0.7):** When overall confidence drops below 0.7, the engine returns `status="disambiguation_required"` with candidate options instead of a resolved answer. The agent must prompt the user for clarification.
...
This is the layer that sits between OpenAI Frontier (agent execution) and Snowflake SVA (in-platform analytics). It is the deep, cross-estate, trustworthy context that makes enterprise AI actually work.
